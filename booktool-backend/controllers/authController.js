...
exports.register = async (req, res, next) => {
    try {
        const { firstName, lastName, role, password, email, phoneNumber } = req.body;
        if(!firstName || !lastName || !role || !password || !email || !phoneNumber) {
            return next(new AppError('Please provide email and password!', 400));
        }

        const user = await User.findOne({ email, phoneNumber }).select('+password');
        if(user) {
            return next(new AppError('User with that information already exits', 400));
        }
